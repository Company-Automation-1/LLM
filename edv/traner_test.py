...
from main_Lib_1 import *
from main_Lib_2 import softplus, sigmoid
from util import pt

#TODO 反向传播

def diff_vect_weight(vectors, 
  ...
  for layer in range(layers):
      ...
      for row in range(rows):
         ...
         for col in range(cols):
            ...
            back_weights[layer][row][col] =  2 * ((softmax(vectors[layers], temperature) - actual).T @ diff_softmax(
               vectors[layers], 
               diff_vect_weight(
                  vectors, 
                  intermediate_vectors, 
      ...
      for row in range(rows):
         ...
         for col in range(cols):
            ...
            back_act_weights[layer][row][col] =  2 * ((softmax(vectors[layers], temperature) - actual).T @ diff_softmax(
               vectors[layers], 
               diff_vect_aweight(
                  vectors, 
                  intermediate_vectors, 
      ...
      for row in range(rows):
         ...
         back_biases[layer][row] = 2 * ((softmax(vectors[layers], temperature) - actual).T @ diff_softmax(
            vectors[layers], 
            diff_vect_bias(
               vectors, 
               intermediate_vectors, 
